...
function get_with_token(token, endpoint, cb) {
    request(token, 'GET', endpoint/*null*/, null, cb)
}

function get_data(token, endpoint, entry, cb) {
            ...
            if(cb) {
                ...
                if(xhr.statusText === "No Content"){
                    ...
                    if(xhr.status === 204){
                        ...
                        var obj0 = JSON.parse('{ "del":"successfull delete operation"}');
                        cb(obj0, xhr)
                        return
                    }
                    //-- operation is not successfull --//
                        ...
                        var obj1 = JSON.parse('{ "del":"delete operation was not successfull"}');
                        cb(obj1, xhr)
                        return
